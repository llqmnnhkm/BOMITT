<?php
// admin/admin_includes/handlers/cables_handler.php
// AJAX-only handler for cables & accessories CRUD operations
...
try {
    switch ($action) {
        case 'add_cable':
            ...
            $validation = validateRequiredFields(['category', 'name', 'quantity', 'price']);
            if ($validation !== true) {
            ...
            $result = insertRecord($conn, 'network_cables_accessories', [
            ...
            if ($result['success']) {
                logAdminAction('ADD_CABLE', "Added: {$_POST['name']}");
        ...
        case 'update_cable':
            ...
            $result = updateRecord($conn, 'network_cables_accessories', [
                'item_name' => sanitizeInput($_POST['name']),
                'item_description' => sanitizeInput($_POST['description'] ?? ''),
                'default_quantity' => (int)$_POST['quantity'],
                'unit_price' => (float)$_POST['price']
            ...
            if ($result['success']) {
                logAdminAction('UPDATE_CABLE', "Updated ID: {$_POST['id']} - {$_POST['name']}");
        ...
        case 'delete_cable':
            ...
            if (!isset($_POST['id']) || !isValidNumber($_POST['id'], 1)) {
                jsonError('Invalid cable/accessory ID');
            }
            
            // Delete record using utility
            $result = deleteRecord($conn, 'network_cables_accessories', (int)$_POST['id']);
            
            if ($result['success']) {
                logAdminAction('DELETE_CABLE', "Deleted ID: {$_POST['id']}");
                jsonSuccess($result['message']);
            } else {
                jsonError($result['message']);
            }
